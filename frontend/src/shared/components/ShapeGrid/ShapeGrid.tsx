...
const ShapeGrid: React.FC<ShapeGridProps> = ({
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      // Cache gradient on resize to save CPU cycles
    ...
    const drawGrid = () => {
      ...
      if (isHex) {
        ...
        // 1. Draw base grid lines in a single batch
        // 1. Draw base grid lines in a single batch using continuous lines (FLASH smooth)
        ctx.strokeStyle = borderColor;
        ctx.beginPath();
        ...
        for (let col = -2; col < cols + 1; col++) {
          ...
          ctx.moveTo(x, -squareSize + offsetY);
          ctx.lineTo(x, (rows - 1) * squareSize + offsetY);
        }
        
        // Horizontal lines
        for (let row = -2; row < rows + 1; row++) {
          const y = row * squareSize + offsetY;
          ctx.moveTo(-squareSize + offsetX, y);
          ctx.lineTo((cols - 1) * squareSize + offsetX, y);
        }
        ctx.stroke();

        // 2. Draw hover fills only for active cells (very efficient)
    ...
    const updateAnimation = () => {
      ...
      // Process mouse position in the animation loop for smoothness
      if (mousePos.current) {
    ...
    const updateCellOpacities = () => {
      ...
      for (const [key, opacity] of cellOpacities.current) {
        ...
        const next = opacity + (target - opacity) * 0.25;
        if (next < 0.005) {
